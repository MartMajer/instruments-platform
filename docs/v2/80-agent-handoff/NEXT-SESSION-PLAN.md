...
D360 is deployed at app commit `4dc0e0c`. It rejects Auth0 callbacks where the provider returns a different email than the expected `/signin` email and shows `/app?auth=email_mismatch` wrong-account recovery. D359 fixed the mobile shell regression where `/app` could hydrate into only the bottom nav because duplicate keyed bottom-nav hrefs raised Svelte `each_key_duplicate`, and it made `/signin` plus `/register` action-first on phones. D358 remains the first real mobile navigation replacement: public home menu, app top bar, bottom nav, More sheet, selected-study switcher, and neutral card selected states. It is not a full route-by-route screenshot audit.
