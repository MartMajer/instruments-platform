	...
	<LoadingBoundary loading={loadState === 'loading'} label={`Loading ${surface} context`}>
		...
		{:else if setupWorkspaceView}
			...
			<section
				...
				aria-label={setupWorkspaceView.referenceTitle}
			>
				<div class="product-panel__header">
					<div>
						<p class="product-kicker">Reference</p>
						<h2 class="product-title">{setupWorkspaceView.referenceTitle}</h2>
						<p class="mt-1 text-sm text-[var(--color-text-muted)]">
							{setupWorkspaceView.referenceDescription}
						</p>
					</div>
					<StatusBadge
						status={setupWorkspaceView.readiness.status}
				...
				>
					<div>
						...
						<h3 class="text-base font-semibold text-[var(--color-text)]">
							Selected campaign setup state
						</h3>
					</div>

					{#if setupWorkspaceView.selectedCampaignRows.length > 0}
						<dl class="record-grid">
							{#each setupWorkspaceView.selectedCampaignRows as row}
								<div class="record-field">
									<dt class="record-field__label">{row.label}</dt>
					...
					{:else if setupWorkspaceView.emptyState}
						<p class="record-row text-sm text-[var(--color-text-muted)]">
							<strong class="record-row__title">{setupWorkspaceView.emptyState.title}</strong>
							<span>{setupWorkspaceView.emptyState.message}</span>
						</p>
					{/if}
				</div>
				...
				<div
					role="group"
					aria-label="Setup policies"
					class="grid gap-3 border-t border-[var(--color-border)] pt-4"
				>
					<div>
						<p class="product-kicker">Policies</p>
						<h3 class="text-base font-semibold text-[var(--color-text)]">
							Consent, retention, and disclosure
						</h3>
					</div>
					<div class="record-grid">
						{#each setupWorkspaceView.policyRows as row}
							<div class="record-row">
								<div class="record-row__header">
									...
									<StatusBadge status={row.status} label={row.badgeLabel} />
								</div>
								{#if row.details.length > 0}
									<dl class="record-grid">
										{#each row.details as detail}
											<div class="record-field">
												<dt class="record-field__label">{detail.label}</dt>
												<dd class="record-field__value">{detail.value}</dd>
											</div>
										{/each}
									</dl>
								{/if}
				...
				<div
					role="group"
					aria-label="Template and scoring"
					class="grid gap-3 border-t border-[var(--color-border)] pt-4"
				>
					<div>
						<p class="product-kicker">Instrument execution</p>
						<h3 class="text-base font-semibold text-[var(--color-text)]">
							Template and scoring state
						</h3>
					</div>
					<div class="record-grid">
						<article class="record-row" aria-label="Selected template">
							<div class="record-row__header">
								<h4 class="record-row__title">Template</h4>
							</div>
							{#if setupWorkspaceView.templateRows.length > 0}
								<dl class="record-grid">
									{#each setupWorkspaceView.templateRows as row}
										<div class="record-field">
											<dt class="record-field__label">{row.label}</dt>
											<dd class="record-field__value">{row.value}</dd>
										</div>
							...
							{:else}
								<p class="text-sm text-[var(--color-text-muted)]">Missing</p>
							{/if}
						</article>

						<article class="record-row" aria-label="Selected scoring rule">
							<div class="record-row__header">
								<h4 class="record-row__title">Scoring</h4>
							</div>
							{#if setupWorkspaceView.scoringRows.length > 0}
								<dl class="record-grid">
									{#each setupWorkspaceView.scoringRows as row}
										<div class="record-field">
											<dt class="record-field__label">{row.label}</dt>
											<dd class="record-field__value">{row.value}</dd>
										</div>
				...
				{#if setupWorkspaceView.missingPrerequisiteRows.length > 0}
					<div
						role="group"
						aria-label="Missing setup prerequisites"
						class="grid gap-3 border-t border-[var(--color-border)] pt-4"
					>
						<div>
						...
						<div class="record-list">
							{#each setupWorkspaceView.missingPrerequisiteRows as row}
				...
				<div class="grid gap-3 border-t border-[var(--color-border)] pt-4">
					<div>
						...
						<h3 class="text-base font-semibold text-[var(--color-text)]">Setup campaign context</h3>
					</div>

					{#if setupWorkspaceView.emptyState}
						<p class="record-row text-sm text-[var(--color-text-muted)]">
							<strong class="record-row__title">{setupWorkspaceView.emptyState.title}</strong>
							<span>{setupWorkspaceView.emptyState.message}</span>
						</p>
					{:else}
						<div class="record-list">
							{#each setupWorkspaceView.campaignRows as campaign (campaign.id)}
