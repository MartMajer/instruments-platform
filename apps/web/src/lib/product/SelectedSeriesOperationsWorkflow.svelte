<script lang="ts">
	...
	import type {
		...
		CampaignIdentifiedQueueAccessRespondentResponse,
	...
	const operationsWorkflowCopy = $derived(
		routePageCopy(appLocale).selectedStudy.operationsWorkflow
	);
	...
	let copiedQueueRespondentId = $state<string | null>(null);
	...
	const providerComplainedEventCount = $derived(
		selectedCampaign?.providerComplainedEventCount ?? 0
	);
	...
	const locallyFailedInvitationCount = $derived(
		localFailedInvitationOverride ?? failedInvitationCount
	);
	...
	const readinessIssueGuidance = $derived(
		readinessResult?.issues.length ? readinessResult.issues.map(toReadinessIssueGuidance) : []
	);

	$effect(() => {
		if (selectedCampaignIsIdentified && shareAccessMode === 'email') {
			shareAccessMode = 'openLink';
		}
	});
	...
	async function createRespondentAccess() {
		...
		if (!selectedCampaignIsIdentified && emailInviteAccessActive) {
			actionErrors = {
				...actionErrors,
				openLink: operationsBodyCopy.component.errors.privateInvitationsAlreadyActive
			};
			return;
		}
		...
		if (!selectedCampaignIsIdentified && openLinkAccessActive) {
			actionErrors = {
				...actionErrors,
				openLink: operationsBodyCopy.component.errors.openLinkAlreadyActive
			};
			return;
		}
	...
	async function createIdentifiedQueueAccess() {
		if (!selectedCampaign) {
			actionErrors = {
				...actionErrors,
				openLink: operationsBodyCopy.component.errors.createWaveBeforeAccess
	...
	async function replaceOpenRespondentLink() {
		...
		if (selectedCampaignIsIdentified || !openLinkAccessActive || emailInviteAccessActive) {
			actionErrors = {
				...actionErrors,
				openLink: operationsBodyCopy.component.errors.replaceLinkOnlyForAnonymous
			};
			return;
		}
	...
	async function createEmailInvitations() {
		...
		if (!selectedCampaignSupportsEmailInvites) {
			actionErrors = {
				...actionErrors,
				openLink: operationsBodyCopy.component.errors.emailInvitationsRequireAnonymous
			};
			return;
		}
		...
		if (openLinkAccessActive) {
			actionErrors = {
				...actionErrors,
				openLink: operationsBodyCopy.component.errors.openLinkBlocksPrivateInvitations
			};
			return;
		}
		...
		if (!canCreateEmailInvitations) {
			actionErrors = {
				...actionErrors,
				openLink: operationsBodyCopy.component.errors.reviewRecipientsBeforeInviting
			};
			return;
		}
		...
		try {
			...
			recipientImportFileError =
				error instanceof Error
					? error.message
					: operationsBodyCopy.component.errors.recipientFileCouldNotRead;
		}
	}
	...
	async function retryFailedEmails() {
		...
		if (!retryFailedDeliveryAcknowledged) {
			actionErrors = {
				...actionErrors,
				openLink: operationsBodyCopy.component.errors.confirmRetryBeforeRequeueing
			};
			return;
		}
		if (!emailReadinessResult) {
	...
	async function refreshCollectionStatus() {
		...
		try {
			...
			actionErrors = {
				...actionErrors,
				monitor: toProductApiErrorMessage(
					error,
					operationsBodyCopy.component.errors.collectionStatusRefreshFailed
				)
			};
		}
	}
	...
	async function runAction<T>(
		...
		try {
			...
			actionErrors = {
				...actionErrors,
				[actionId]: toProductApiErrorMessage(
					error,
					operationsBodyCopy.component.errors.collectionActionFailed
				)
			};
			return null;
		}
	...
	function formatAssignmentCount(value: number) {
		return operationsBodyCopy.shareAccess.assignmentCount(value, countFormatter.format(value));
	}

	function queueRespondentPath(respondent: CampaignIdentifiedQueueAccessRespondentResponse) {
		return respondent.respondentPath || operationsBodyCopy.shareAccess.linkNotDisplayed;
	}

	function queueRespondentUrl(respondent: CampaignIdentifiedQueueAccessRespondentResponse) {
		const path = respondent.respondentPath;
		if (!path) {
			return null;
		}
	...
	async function copyIdentifiedQueueLink(
		respondent: CampaignIdentifiedQueueAccessRespondentResponse
	) {
		const url = queueRespondentUrl(respondent);
		if (!url) {
			return;
		}
		...
		try {
			...
			actionErrors = {
				...actionErrors,
				openLink: toProductApiErrorMessage(error, operationsBodyCopy.shareAccess.copyFailed)
			};
		}
	}

	function downloadIdentifiedQueueCsv() {
		const result = identifiedQueueAccessResult;
		if (!result) {
			return;
		}
		...
		const lines = [
			...
			...result.respondents.map((respondent) =>
				[
					respondent.respondentLabel,
					respondent.respondentEmail ?? '',
					String(respondent.assignmentCount),
					respondent.accessStatus,
					respondent.respondentPath ?? ''
				]
					.map(toCsvCell)
					.join(',')
			)
		...
		anchor.download = `${toSafeFileName(selectedCampaign?.name ?? 'collection-wave')}-queue-links.csv`;
	...
	function emailReadinessBadgeLabel() {
		...
		if (emailReadinessResult.canSendRealEmail) {
			return emailReadinessResult.webhookConfigured
				? operationsBodyCopy.component.emailStatus.smtpReady
				: operationsBodyCopy.component.emailStatus.smtpSendReady;
		}

		return emailReadinessResult.mode === 'local_dev'
			? operationsBodyCopy.component.emailStatus.localProofMode
			: operationsBodyCopy.component.emailStatus.needsConfig;
	}
	...
	function toReadinessIssueGuidance(issue: ReadinessIssue): ReadinessIssueGuidance {
		...
		if (code === 'campaign.status_not_launchable') {
			return {
				...
				detail: operationsBodyCopy.component.readinessGuidance.useDraftWaveDetail,
		...
		if (code === 'identity.unknown') {
			return {
				...
				detail: operationsBodyCopy.component.readinessGuidance.chooseResponseModeDetail,
				severity: issue.severity
			};
		}
		...
		if (code === 'template_version.missing') {
			return {
				...
				detail: operationsBodyCopy.component.readinessGuidance.connectQuestionnaireDetail,
				severity: issue.severity
			};
		}
		...
		if (code.startsWith('template.')) {
			return {
				...
				detail: operationsBodyCopy.component.readinessGuidance.finishQuestionnaireDetail,
				severity: issue.severity
			};
		}
		...
		if (code.startsWith('scoring_rule.') || normalized.includes('scoring')) {
			return {
				...
				detail: operationsBodyCopy.component.readinessGuidance.finishResultsDetail,
				severity: issue.severity
			};
		}
		...
		if (
			...
			return {
				...
				detail: operationsBodyCopy.component.readinessGuidance.completePoliciesDetail,
				severity: issue.severity
			};
		}
		...
		if (code === 'respondent_rule.identity_mode_not_supported') {
			return {
				...
				detail: operationsBodyCopy.component.readinessGuidance.switchResponseModeDetail,
				severity: issue.severity
			};
		}
		...
		if (code === 'respondent_rule.email_required') {
			return {
				...
				detail: operationsBodyCopy.component.readinessGuidance.addEmailsDetail,
				severity: issue.severity
			};
		}
		...
		if (code === 'respondent_rule.no_recipients') {
			return {
				...
				detail: operationsBodyCopy.component.readinessGuidance.selectRecipientDetail,
				severity: issue.severity
			};
		}
		...
		if (code === 'respondent_rule_preview.audience_missing') {
			return {
				...
				detail: operationsBodyCopy.component.readinessGuidance.emptyRecipientDetail,
				severity: issue.severity
			};
		}
		...
		if (code.startsWith('respondent_rule_preview.')) {
			return {
				...
				detail: operationsBodyCopy.component.readinessGuidance.fixAudienceDetail,
				severity: issue.severity
			};
		}
		...
		if (code.startsWith('instrument.')) {
			return {
				...
				detail: operationsBodyCopy.component.readinessGuidance.reviewInstrumentDetail,
				severity: issue.severity
			};
		}
...
<section class="product-panel" role="group" aria-label={operationsBodyCopy.progressAriaLabel}>
	...
	{:else}
		<article
			...
			role="region"
			aria-label={operationsBodyCopy.stepAriaLabel}
		>
			...
			<div class="setup-current-task__body">
					...
					<dl class="record-grid">
						<div class="record-field">
							...
							<dd class="record-field__value">
								{selectedCampaign?.name ?? operationsBodyCopy.common.missing}
							</dd>
						</div>
						<div class="record-field">
				...
				{:else if activeAction.id === 'openLink'}
					...
					<div class="record-row">
						<div class="record-row__header">
							<div>
								<p class="record-field__label">
									{operationsBodyCopy.shareAccess.accessMethodLabel}
								</p>
								<h5 class="record-row__title">
									{operationsBodyCopy.shareAccess.chooseAccessMethodTitle}
								</h5>
							</div>
							<StatusBadge
								status={respondentAccessPrepared ? 'ready' : 'neutral'}
								label={respondentAccessPrepared
						...
						<div
							class="action-row"
							role="group"
							aria-label={operationsBodyCopy.shareAccess.accessMethodLabel}
						>
							<button
								type="button"
							...
							>
								...
								<span>
									{selectedCampaignIsIdentified
										? operationsBodyCopy.shareAccess.queueLinksMode
										: operationsBodyCopy.shareAccess.openLinkMode}
								</span>
							</button>
							{#if !selectedCampaignIsIdentified}
								<button
					...
					<details class="record-row">
						<summary class="record-row__title"
							>{operationsBodyCopy.shareAccess.advancedDeliveryControls}</summary
						>
						<div class="mt-3 grid gap-3">
							<div class="record-row">
								...
								{#if emailReadinessResult}
									<dl class="record-grid">
										...
										<div class="record-field">
											<dt class="record-field__label">
												{operationsBodyCopy.emailSetup.realEmailSend}
											</dt>
											<dd class="record-field__value">
												{emailReadinessResult.canSendRealEmail
													? operationsBodyCopy.common.available
													: operationsBodyCopy.common.notAvailable}
											</dd>
										</div>
										<div class="record-field">
											<dt class="record-field__label">
												{operationsBodyCopy.emailSetup.providerEvents}
											</dt>
											<dd class="record-field__value">
												{emailReadinessResult.webhookConfigured
													? operationsBodyCopy.emailSetup.webhookConfigured
													: operationsBodyCopy.emailSetup.webhookDisabled}
											</dd>
										</div>
									</dl>
							...
							<div class="record-row">
								<div class="record-row__header">
									<div>
										<p class="record-field__label">
											{operationsBodyCopy.component.doNotContactList}
										</p>
										<h5 class="record-row__title">
											{operationsBodyCopy.component.suppressEmailsBeforeInviting}
										</h5>
									</div>
									<StatusBadge
										status={(suppressionListResult?.activeCount ?? 0) > 0 ? 'pending' : 'neutral'}
										label={suppressionListResult
											? `${formatCount(suppressionListResult.activeCount)} active`
											: operationsBodyCopy.component.notLoaded}
									/>
								</div>
								<p class="text-sm leading-6 text-[var(--color-text-muted)]">
									Add addresses that must not receive campaign invitations from this tenant.
									Suppression is checked when invitations are created, when saved recipient lists
									are launched, and again before email delivery. Releasing an address only allows
									future explicit invitations; it does not resend or restore old suppressed emails.
								</p>
									...
									{#if activeEmailSuppressions.length > 0}
										<div class="grid gap-2">
											...
											{#if activeEmailSuppressions.length > 6}
												<p class="text-sm text-[var(--color-text-muted)]">
													Showing first 6 of {formatCount(activeEmailSuppressions.length)} active do-not-contact
													records.
												</p>
											{/if}
										</div>
								...
								{:else}
									<p class="text-sm text-[var(--color-text-muted)]">
										Refresh the list before a live send when you need to review tenant-level
										suppression records.
									</p>
								{/if}
							</div>
					...
					{#if shareAccessMode === 'email'}
						{#if locallyPreparedInvitationCount > 0}
							<div class="record-row">
								<div class="record-row__header">
									<h5 class="record-row__title">
										{operationsBodyCopy.component.emailInvitationStatus}
									</h5>
								...
								<dl class="record-grid">
									...
									<div class="record-field">
										<dt class="record-field__label">
											{operationsBodyCopy.cleanup.retryableFailures}
										</dt>
										<dd class="record-field__value">
									...
									<div class="record-field">
										<dt class="record-field__label">
											{operationsBodyCopy.component.providerAccepted}
										</dt>
										<dd class="record-field__value">
									...
									<div class="record-field">
										<dt class="record-field__label">
											{operationsBodyCopy.component.providerDelivered}
										</dt>
										<dd class="record-field__value">
									...
									<div class="record-field">
										<dt class="record-field__label">
											{operationsBodyCopy.component.providerBounced}
										</dt>
										<dd class="record-field__value">
									...
									<div class="record-field">
										<dt class="record-field__label">
											{operationsBodyCopy.monitor.latestProviderEvent}
										</dt>
										<dd class="record-field__value">{formatDateTime(latestProviderEventAt)}</dd>
									</div>
								</dl>
								<p class="text-sm text-[var(--color-text-muted)]">
									Sent means the platform handed the email to the configured provider. Provider
									counts show later accepted, delivered, bounced, or complaint events when they have
									been reconciled{providerDeliveryEventCount > 0
										? '.'
										: '; no provider events are recorded yet.'}
						...
						<div class="record-row">
							<div class="record-row__header">
								<div>
									<p class="record-field__label">
										{operationsBodyCopy.component.invitedEmailAccess}
									</p>
									<h5 class="record-row__title">
										{operationsBodyCopy.component.addOneOffRecipientsAfterLaunch}
									</h5>
								</div>
								<StatusBadge
									status={selectedCampaignSupportsEmailInvites && !openLinkAccessActive
										? 'neutral'
										: 'blocked'}
									label={selectedCampaignSupportsEmailInvites && !openLinkAccessActive
							...
							{#if selectedCampaignSupportsEmailInvites}
								{#if openLinkAccessActive}
									<p class="error-line" role="alert">
										This wave already has an open respondent link. Keep using open-link collection
										or create a new wave for private email invitations.
									</p>
								{/if}
								<div class="record-row">
									<div class="record-row__header">
										<div>
											<p class="record-field__label">
												{operationsBodyCopy.component.postLaunchAdditions}
											</p>
											<h6 class="record-row__title">
												{operationsBodyCopy.component.addOneTimeRecipientsToThisWave}
											</h6>
										</div>
										<span
											class="step-pill"
									...
									<label class="field">
										...
										<span class="text-sm text-[var(--color-text-muted)]">
											Use this for late additions or a one-time list after launch. For the normal
											study recipient list, use Directory groups or the saved recipient selection in
											Setup before launch. Review happens in this browser before private invitation
											links are created. Limit: {formatCount(maxRecipientImportRecipients)} recipients
											per wave update.
										</span>
									</label>
									<details>
										<summary class="record-row__title"
											>{operationsBodyCopy.component.reviewOrPasteSourceList}</summary
										>
										<label class="field mt-3">
											...
											></textarea>
											<span class="text-sm text-[var(--color-text-muted)]">
												Use this for copied spreadsheet columns or cleanup after import. One row can
												be an email address or Name &lt;email@example.com&gt;. This does not create
												Directory people or groups.
											</span>
										</label>
									</details>
								</div>
								...
								{#if recipientImportReview.rows.length > 0}
									<div class="record-row">
										<div class="record-row__header">
											...
											<span
												class="step-pill"
												data-state={recipientImportReview.hasBlockingIssues
													? 'failed'
													: 'succeeded'}
											>
												{formatCount(recipientImportReview.validRecipientCount)} ready
										...
										<dl class="record-grid">
											...
											<div class="record-field">
												<dt class="record-field__label">
													{operationsBodyCopy.component.duplicates}
												</dt>
												<dd class="record-field__value">
											...
											{#each recipientImportReview.rows.slice(0, 8) as row (row.id)}
												<div class="record-field">
													...
													<p class="record-field__value">
														{row.email || operationsBodyCopy.component.noEmailFound}
													</p>
								...
								{#if recipientSuppressionReview.hasBlockedRecipients}
									<div class="record-row">
										<div class="record-row__header">
											<div>
												<p class="record-field__label">
													{operationsBodyCopy.component.doNotContactMatch}
												</p>
								...
								<details class="record-row">
									<summary class="record-row__title"
										>{operationsBodyCopy.component.invitationEmailPreview}</summary
									>
									<div class="record-field mt-3">
										...
										<p class="mt-2 text-sm whitespace-pre-wrap text-[var(--color-text-muted)]">
											{emailBody()}
										</p>
									</div>
									<p class="mt-3 text-sm text-[var(--color-text-muted)]">
										Each recipient gets their own link. Delivery status stays operational; reports
										and exports do not expose recipient identity for anonymous or
										repeat-participation waves. Unsubscribe requests add the recipient to this
										workspace's do-not-contact list. Sender identity, footer text, and reminder
										cadence are governed by email delivery settings and future reminder workflow.
									</p>
								</details>
								{#if locallyFailedInvitationCount > 0}
									<div class="field">
										...
										<label
											class="inline-flex items-start gap-2 text-sm text-[var(--color-text-muted)]"
										>
											<input
												type="checkbox"
								...
								{#if deliveryResult}
									...
									{#if deliveryBatchGuidance(deliveryResult)}
										<p
											class={deliveryResult.failedCount > 0
												? 'error-line'
							...
							{:else}
								<p class="text-sm text-[var(--color-text-muted)]">
									Email invitations currently require an anonymous or repeat-participation
									collection wave. Use Setup to change the response mode, or use the access option
									below for this wave.
								</p>
							{/if}
						</div>
						...
						<div class="record-row">
							<div class="record-row__header">
								<div>
									...
									<h5 class="record-row__title">
										{operationsBodyCopy.component.simulateCollectionResponses}
									</h5>
									<p class="text-sm text-[var(--color-text-muted)]">
								...
								<StatusBadge
									...
									label={testResponseResult
										? operationsBodyCopy.component.responsesCreated
										: operationsBodyCopy.component.stagingDemo}
								/>
							</div>
							<div
								class="grid gap-3 lg:grid-cols-[minmax(7rem,10rem)_minmax(7rem,10rem)_minmax(9rem,12rem)_auto]"
							>
								<label class="field">
							...
							{#if testResponseResult}
								<div class="record-grid">
									...
									<div class="record-field">
										<p class="record-field__label">
											{operationsBodyCopy.simulation.scoredResponses}
										</p>
										<p class="record-field__value">
					...
					{#if shareAccessMode === 'openLink'}
						{#if selectedCampaignIsIdentified}
							<section
								class="record-row"
								aria-label={operationsBodyCopy.shareAccess.identifiedQueueAria}
							>
								<div class="record-row__header">
									<div>
										<p class="record-field__label">
											{operationsBodyCopy.shareAccess.identifiedQueueLabel}
										</p>
										<h5 class="record-row__title">
											{operationsBodyCopy.shareAccess.respondentQueueLinks}
										</h5>
									</div>
									<StatusBadge
						...
						{:else}
							<div class="record-row">
								<div class="record-row__header">
									...
									<StatusBadge
										status={emailInviteAccessActive
											? 'blocked'
											: openLinkAccessActive
												? 'ready'
												: 'neutral'}
										label={emailInviteAccessActive
								...
								{:else if openLinkAccessActive}
									<div class="action-row">
										...
										<p class="step-pill" data-state={actionStates.openLink}>
											{actionStates.openLink === 'succeeded'
												? operationsBodyCopy.shareAccess.replaced
												: operationsBodyCopy.shareAccess.oneActiveLink}
										</p>
									</div>
									{#if actionErrors.openLink}
										<p class="error-line">{actionErrors.openLink}</p>
								...
								{:else}
									{@render ActionFooter({
										id: 'openLink',
										label: operationsBodyCopy.shareAccess.createRespondentLink,
							...
							{#if respondentEntry}
								<div class="record-row">
									<div class="record-row__header">
										<h5 class="record-row__title">
											{operationsBodyCopy.shareAccess.respondentLinkReady}
										</h5>
										<span class="step-pill" data-state="succeeded"
											>{operationsBodyCopy.common.created}</span
										>
									</div>
									<p class="result-line">
				...
				{:else if activeAction.id === 'monitor'}
					...
					<dl class="record-grid">
						<div class="record-field">
							<dt class="record-field__label">{operationsBodyCopy.common.started}</dt>
							<dd class="record-field__value">
								{formatCount(workspace.summary.startedResponseCount)}
							</dd>
						</div>
						<div class="record-field">
							<dt class="record-field__label">{operationsBodyCopy.common.inProgress}</dt>
							<dd class="record-field__value">
								{formatCount(workspace.summary.draftResponseCount)}
							</dd>
						</div>
						<div class="record-field">
						...
						<div class="record-field">
							...
							<dd class="record-field__value">
								{humanize(workspace.summary.reportVisibilityStatus)}
							</dd>
						</div>
					</dl>
					<div class="record-row">
						<div class="record-row__header">
							...
							<StatusBadge
								...
								label={providerDeliveryEventCount > 0
					...
					<div class="record-row">
						<div class="record-row__header">
							...
							<StatusBadge
								status={repairReadinessResult?.hasRepairWork
									? 'pending'
									: repairReadinessResult
										? 'ready'
										: 'neutral'}
								label={repairReadinessResult?.hasRepairWork
						...
						{#if repairReadinessResult}
							...
							<dl class="record-grid">
								...
								<div class="record-field">
									<dt class="record-field__label">
										{operationsBodyCopy.cleanup.ambiguousFailures}
									</dt>
									<dd class="record-field__value">
								...
								<div class="record-field">
									<dt class="record-field__label">
										{operationsBodyCopy.cleanup.suppressedFailures}
									</dt>
									<dd class="record-field__value">
						...
						<div class="action-row">
							...
							{#if repairReadinessResult?.canRetryFailed}
								<p class="step-pill" data-state="pending">
									{operationsBodyCopy.cleanup.retryPossible}
								</p>
							{/if}
						</div>
					</div>
				...
				<div class="action-row" aria-label={operationsBodyCopy.navigation.ariaLabel}>
					...
					{#if activeAction.id === 'close'}
						<a class="secondary-button" href={resultsHref}
							>{operationsBodyCopy.navigation.goToResults}</a
						>
					{:else}
						<button
							type="button"
