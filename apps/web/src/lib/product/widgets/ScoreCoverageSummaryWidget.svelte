...
<ReportWidgetShell {widget} {copy}>
	{#if data}
		<dl class="record-grid">
			...
			<div class="metric-card">
				<dt class="metric-card__label">{formatWidgetLabel('scored', copy)}</dt>
				<dd class="metric-card__value">{data.scoredSubmittedResponseCount}</dd>
			</div>
			<div class="metric-card">
				<dt class="metric-card__label">{formatWidgetLabel('unscored', copy)}</dt>
				<dd class="metric-card__value">{data.unscoredSubmittedResponseCount}</dd>
			</div>
			<div class="metric-card">
				<dt class="metric-card__label">{formatWidgetLabel('notConfigured', copy)}</dt>
		...
		<div class="record-grid">
			<div class="record-field">
				<dt class="record-field__label">{formatWidgetLabel('coverageStatus', copy)}</dt>
				<dd class="record-field__value">{formatCodeLabel(data.status, copy)}</dd>
			</div>
			<div class="record-field">
				<dt class="record-field__label">{formatWidgetLabel('latestScoringActivity', copy)}</dt>
				<dd class="record-field__value">{formatNullableDate(data.latestScoringActivityAt, copy)}</dd>
			</div>
		</div>
	{:else}
		<p class="text-sm text-[var(--color-text-muted)]">
			{formatWidgetLabel('scoreCoverageDataUnavailable', copy)}
		</p>
	{/if}
</ReportWidgetShell>
