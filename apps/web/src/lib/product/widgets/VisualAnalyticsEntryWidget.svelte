<script lang="ts">
	...
	type DetailPanel = 'outputs' | 'groups' | 'waves' | 'insights';
	type SortState<T extends string> = {
		key: T;
		direction: SortDirection;
	};

	let activeDetailPanel = $state<DetailPanel>('outputs');
	...
	function nextSort<T extends string>(current: SortState<T>, key: T): SortState<T> {
		if (current.key !== key) {
			return {
				key,
				direction: key === 'result' || key === 'group' || key === 'campaign' ? 'asc' : 'desc'
			};
		}
			...
			switch (sort.key) {
				...
				case 'range':
					return compareNullableNumber(
						rangeWidth(left.min, left.max),
						rangeWidth(right.min, right.max),
						sort.direction
					);
				case 'missingness':
					return compareText(
						left.missingPolicyStatusSummary ?? '',
						right.missingPolicyStatusSummary ?? '',
						sort.direction
					);
			}
		});
	...
	function sortGroupRows(
		rows: CampaignSeriesResultsGroupMatrixRowResponse[],
		...
		return [...rows].sort((left, right) => {
			...
			switch (sort.key) {
				case 'group':
					return compareText(
						`${left.groupType} ${left.groupName}`,
						`${right.groupType} ${right.groupName}`,
						sort.direction
					);
				case 'result':
					return compareText(formatResultName(left), formatResultName(right), sort.direction);
	...
	function sortWaveRows(
		rows: CampaignSeriesResultsWaveMatrixRowResponse[],
		sort: SortState<WaveSortKey>
	) {
		return [...rows].sort((left, right) => {
			const disclosure = compareDisclosure(left.disclosure, right.disclosure);
			if (disclosure !== 0) {
			...
			switch (sort.key) {
				...
				case 'previous':
					return compareNullableNumber(
						left.deltaFromPreviousMean,
						right.deltaFromPreviousMean,
						sort.direction
					);
				case 'first':
					return compareNullableNumber(
						left.deltaFromFirstMean,
						right.deltaFromFirstMean,
						sort.direction
					);
			}
		});
	...
	function insightEvidence(insight: CampaignSeriesResultsInsightResponse) {
		if (insight.kind === 'score_outputs') {
			...
			return {
				...
				value: output
					? `${formatResultName(output)} / ${formatScore(output.mean)}`
	...
	function formatResultName(
		value: string | { dimensionCode: string; displayLabel?: string | null }
	) {
		...
		{#if analytics}
			<dl class="record-grid" aria-label={formatWidgetLabel('highlights', copy)}>
				<div class="metric-card">
					...
					<dd class="metric-card__value">
						{highestOutput
							? formatScore(highestOutput.mean)
							: (copy?.notAvailable ?? 'Not available')}
					</dd>
					{#if highestOutput}
			...
			<div
				class="results-detail-tabs"
				role="tablist"
				aria-label={formatWidgetLabel('resultsDetailTabs', copy)}
			>
				<button
					type="button"
					class="results-detail-tabs__button"
					data-selected={activeDetailPanel === 'outputs'}
					role="tab"
					aria-selected={activeDetailPanel === 'outputs'}
				...
				<button
					type="button"
					class="results-detail-tabs__button"
					data-selected={activeDetailPanel === 'groups'}
					role="tab"
					aria-selected={activeDetailPanel === 'groups'}
				...
				<button
					type="button"
					class="results-detail-tabs__button"
					data-selected={activeDetailPanel === 'waves'}
					role="tab"
					aria-selected={activeDetailPanel === 'waves'}
				...
				<button
					type="button"
					class="results-detail-tabs__button"
					data-selected={activeDetailPanel === 'insights'}
					role="tab"
					aria-selected={activeDetailPanel === 'insights'}
					onclick={() => setDetailPanel('insights')}
				>
					{formatWidgetLabel('insights', copy)}
					<span>{analytics.insights.length}</span>
				</button>
			</div>

			{#if activeDetailPanel === 'insights'}
									...
									<tr>
										...
										<th>
											<button
												type="button"
												class="matrix-sort-button"
												onclick={() => toggleGroupSort('result')}
											>
										...
										<th>
											<button
												type="button"
												class="matrix-sort-button"
												onclick={() => toggleGroupSort('sample')}
											>
										...
										<th>
											<button
												type="button"
												class="matrix-sort-button"
												onclick={() => toggleGroupSort('median')}
											>
									...
									<tr>
										<th>
											<button
												type="button"
												class="matrix-sort-button"
												onclick={() => toggleWaveSort('campaign')}
											>
										...
										<th>
											<button
												type="button"
												class="matrix-sort-button"
												onclick={() => toggleWaveSort('previous')}
											>
												{formatWidgetLabel('fromPrevious', copy)}{sortIndicator(
													waveSort,
													'previous'
												)}
											</button>
