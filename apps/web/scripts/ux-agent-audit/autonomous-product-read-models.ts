...
function campaignSeriesHub(seriesId: string) {
	...
	return {
		...
		governance: {
			consentStatus:
				seriesId === autonomousProductPaths.ownStudySeriesId ? 'not_configured' : 'configured',
			retentionStatus:
				seriesId === autonomousProductPaths.ownStudySeriesId ? 'not_configured' : 'configured',
			disclosureStatus:
				seriesId === autonomousProductPaths.ownStudySeriesId ? 'not_configured' : 'configured',
		...
		lifecycle: [
			lifecycleItem(
				'setup',
				seriesId === autonomousProductPaths.ownStudySeriesId ? 'not_configured' : 'ready'
			),
			...
			lifecycleItem(
				'waves',
				seriesId === autonomousProductPaths.longitudinalSampleSeriesId
					? 'proof_only'
					: 'not_available'
			)
		],
		campaigns:
			seriesId === autonomousProductPaths.ownStudySeriesId ? [] : [campaignSummary(seriesId)],
		archived: false,
		archivedAt: null,
...
function setupWorkspace(seriesId: string) {
	const empty =
		seriesId === autonomousProductPaths.ownStudySeriesId ||
		seriesId === autonomousProductPaths.setupSampleSeriesId;
...
function operationsWorkspace(seriesId: string) {
	const empty =
		seriesId === autonomousProductPaths.ownStudySeriesId ||
		seriesId === autonomousProductPaths.setupSampleSeriesId;
	const submittedResponseCount = empty
		? 0
		: seriesId === autonomousProductPaths.collectionSampleSeriesId
			? 0
			: 128;
...
function reportsWorkspace(seriesId: string) {
	...
	return {
		...
		summary: {
			...
			closedWaveReportCount: isLongitudinal
				? 2
				: seriesId === autonomousProductPaths.completedSampleSeriesId
					? 1
					: 0
		},
		selectedCampaign,
		...
		widgets: [
			{
				...
				message: reports.selectedCampaign
					? null
					: 'Collect and score responses before result cards can be shown.',
				data: {
					dashboard: resultsDashboard(reports, seriesId)
				},
				dataSource: null,
				actions: []
			},
			{
				...
				message: reports.selectedCampaign
					? null
					: 'Collect and score responses before detailed result tables can be shown.',
				data: {
					selectedCampaignId: reports.selectedCampaign?.id ?? null,
					...
					analytics: resultsAnalytics(reports, seriesId)
				},
				dataSource: null,
				actions: []
			},
			{
				...
				message: reports.selectedCampaign
					? null
					: 'Collect and score responses before results can be reviewed.',
				data: {
...
function resultsDashboard(reports: ReturnType<typeof reportsWorkspace>, seriesId: string) {
	const analytics = resultsAnalytics(reports, seriesId);
	return {
		selectedCampaignId: reports.selectedCampaign?.id ?? null,
		selectedCampaignName: reports.selectedCampaign?.name ?? null,
		disclosureKMin: 5,
		disclosureState: reports.selectedCampaign ? 'visible' : 'not_available',
		metrics: [
			{
				id: 'visible_outputs',
				value: analytics?.scoreOutputs.filter((row) => row.disclosure === 'visible').length ?? 0,
				unit: 'count',
				detail: 'Disclosure-safe result outputs are visible in the workbench.',
				tone: reports.summary.visibleScoreCount > 0 ? 'good' : 'warning'
			},
			{
				id: 'hidden_outputs',
				value: analytics?.scoreOutputs.filter((row) => row.disclosure !== 'visible').length ?? 0,
				unit: 'count',
				detail: 'Result outputs hidden by disclosure policy.',
				tone: reports.summary.suppressedScoreCount > 0 ? 'warning' : 'good'
			},
			{
				id: 'group_rows',
				value: analytics?.groupRows.filter((row) => row.disclosure === 'visible').length ?? 0,
				unit: 'count',
				detail: 'Visible group comparison rows available for review.',
				tone: 'neutral'
			},
			{
				id: 'compared_measurements',
				value:
					analytics?.waveRows.filter(
						(row) => row.comparisonState === 'compared' || row.comparisonState === 'baseline'
					).length ?? 0,
				unit: 'count',
				detail: 'Measurement rounds included in the trend view.',
				tone: 'neutral'
			}
		],
		outputBars:
			analytics?.scoreOutputs.map((row) => ({
				id: `output:${row.dimensionCode}`,
				label: row.displayLabel ?? row.dimensionCode,
		...
		notes: reports.selectedCampaign
			? [
					{
						kind: 'interpretation_boundary',
						severity: 'info',
						title: 'Interpretation boundary',
						detail:
							'These are tenant-owned aggregate scores, not validated clinical interpretation.'
					}
				]
			: []
	};
...
function resultsAnalytics(reports: ReturnType<typeof reportsWorkspace>, seriesId: string) {
	if (!reports.selectedCampaign) {
		return null;
	}
	...
	const scoreOutputs = [
		resultScoreOutput({
			...
			displayLabel: 'Burnout total',
			mean: isLongitudinal ? 2.4 : 3.1,
			median: isLongitudinal ? 2.3 : 3.2,
			standardDeviation: 0.6,
			min: 1.4,
			max: 4.6,
			scoreCount: reports.summary.visibleScoreCount
		}),
		resultScoreOutput({
			dimensionCode: 'recovery.capacity',
			displayLabel: 'Recovery capacity',
			mean: isLongitudinal ? 3.7 : 3.4,
			median: isLongitudinal ? 3.8 : 3.5,
			standardDeviation: 0.5,
			min: 2.0,
			max: 4.8,
			scoreCount: 104,
			calculation: 'weighted_mean',
			calculationLabel: 'Weighted mean'
		}),
		resultScoreOutput({
			dimensionCode: 'small_group_signal',
			displayLabel: 'Small group signal',
			...
			suppressionReason: 'below_disclosure_k'
		})
	];
	const groupRows = [
		resultGroupRow({
			...
			dimensionCode: 'burnout.total',
			displayLabel: 'Burnout total',
			mean: isLongitudinal ? 2.6 : 3.3,
			median: isLongitudinal ? 2.6 : 3.4,
			standardDeviation: 0.5,
			min: 1.6,
			max: 4.4,
		...
		resultGroupRow({
			...
			dimensionCode: 'burnout.total',
			displayLabel: 'Burnout total',
			mean: isLongitudinal ? 2.1 : 2.8,
			median: isLongitudinal ? 2.1 : 2.9,
			standardDeviation: 0.4,
			min: 1.3,
			max: 4.0,
			submittedResponseCount: 51,
			scoreCount: 49
		}),
		resultGroupRow({
			groupType: 'department',
			groupName: 'Small team',
			dimensionCode: 'burnout.total',
			displayLabel: 'Burnout total',
			disclosure: 'suppressed',
			submittedResponseCount: null,
	...
	const waveRows = isLongitudinal
		? [
				...
				resultWaveRow({
					...
					submittedResponseCount: 6,
					scoreCount: 6,
					deltaFromPreviousMean: -0.3,
					deltaFromFirstMean: -0.3,
		...
		: [
				resultWaveRow({
					...
					deltaFromPreviousMean: null,
					deltaFromFirstMean: null,
					comparisonState: 'not_available'
				})
			];

	return {
		selectedCampaignId: reports.selectedCampaign.id,
		selectedCampaignName: reports.selectedCampaign.name,
		disclosureKMin: 5,
		...
		insights: [
			{
				kind: 'score_outputs',
				severity: 'info',
				title: 'Score outputs available',
				detail:
					'Review visible scores with their calculation method and configured range before interpretation.'
			},
			{
				kind: 'disclosure',
				severity: 'warning',
				title: 'Disclosure still applies',
				detail:
					'Suppressed rows hide values and counts when the group is below the configured threshold.'
			}
		]
	};
}

function resultScoreOutput(
	overrides: Partial<ReturnType<typeof resultScoreOutputDefaults>> & {
		dimensionCode: string;
		displayLabel: string;
	}
) {
	return {
		...resultScoreOutputDefaults(),
		...overrides
	};
}

function resultScoreOutputDefaults() {
	return {
		disclosure: 'visible',
		submittedResponseCount: 128,
		scoreCount: 115,
		...
		nValidTotal: 115,
		nExpectedTotal: 128,
		missingPolicyStatusSummary: '13 skipped or incomplete score inputs',
...
function resultGroupRow(
	overrides: Partial<ReturnType<typeof resultGroupRowDefaults>> & {
		groupType: string;
		groupName: string;
		dimensionCode: string;
		displayLabel: string;
	...
	return {
		...resultGroupRowDefaults(),
		...overrides
	};
}

function resultGroupRowDefaults() {
	return {
		disclosure: 'visible',
		submittedResponseCount: 64,
		scoreCount: 60,
...
function resultWaveRow(
	overrides: Partial<ReturnType<typeof resultWaveRowDefaults>> & {
		campaignId: string;
		campaignName: string;
	}
) {
	return {
		...resultWaveRowDefaults(),
		...overrides
	};
}

function resultWaveRowDefaults() {
	return {
...
function exportArtifact(seriesId = autonomousProductPaths.completedSampleSeriesId) {
	...
	return {
		id: isLongitudinal
			? 'e493fd91-9f78-4bb5-b0ea-55e13f4e71e9'
			: '8e592f74-d0ca-4204-aead-fb00e9e5085a',
		campaignSeriesId: seriesId,
		targetKind: 'campaign',
		targetId: campaignId,
		targetLabel,
...
function lifecycleItem(id: 'setup' | 'operations' | 'reports' | 'waves', status: string) {
	...
	return {
		...
		status,
		guidance:
			status === 'ready' || status === 'proof_only'
